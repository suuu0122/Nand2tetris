...
* [複合ゲート](#complex-gate)
* [ハードウェア記述言語（HDL）](#hdl)
* [NAND・NOT・AND・OR](#basic-gate)
* [マルチプレクサ](#multiplexor)
* [デマルチプレクサ](#demultiplexor)
* [多ビットの基本ゲート](#multi-bit)
* [多入力の基本ゲート](#multi-input)
* [実装リファレンス](#reference)
...
* マルチプレクサとは
	* マルチプレクサは、3入力のゲートである.
	* 「データビット」と呼ばれる2つの入力から1つを選択して出力するために、「選択ビット」と呼ばれる入力が1つ用いられる.
...
* [マルチプレクサの役割](https://contents.zaikostore.com/semiconductor/5479/)
	* マルチプレクサの役割は、「欲しい信号を選択する」こと.
	* マルチプレクサと呼ばれるデバイスは、多重変換装置や多重化装置、合成器などと呼ばれる.
	* 多重化とは、「複数のデータを合成することで、一本の伝送路のみを使用してデータ送信する」こと.
	* アナログ回路でもデジタル回路でも、一つのデバイスにはいくつもの信号が入力されるが、通常使用できるラインは一つである.
	* マルチプレクサを搭載させることで、伝送ラインは共有しながらも、入力信号を任意に合成または入力信号から任意の信号を選択し、出力信号を決定させる.
	* 逆に1つの信号から複数信号へと変換させる装置はデマルチプレクサと呼ばれる.
* [マルチプレクサ回路の実装](https://cognicull.com/ja/xfs3zjce)
	![マルチプレクサ](../img/mux.png)
<br />

<a id="demultiplexor"></a>

## デマルチプレクサ
* デマルチプレクサとは
	* デマルチプレクサは、マルチプレクサとは反対のことを行う.
	* 1つの入力を、選択ビットに従って、2つの可能な出力のどちらかに振り分ける.
		| sel | a | b |
		| --- | - | - |
		| 0   | in | 0 |
		| 1   | 0 | in |

* [デマルチプレクサの役割](https://contents.zaikostore.com/semiconductor/5979/)
	* デマルチプレクサの役割は、「分解したデータを選り分け、適切な出力チャンネルへ伝送する」こと.
	* デマルチプレクサは、その役割から「セレクタ」と呼ばれることもある.
	* デマルチプレクサは、マルチプレクサから受け取った信号を、それぞれのデバイスごとに適切な信号に分解し送り込む. そのため、マルチプレクサから見ると信号の受信側に設置される.
	* マルチプレクサとデマルチプレクサは、セットで使用されることがほとんどで、マルチプレクサとデマルチプレクサを合わせてマルチプレクサと呼ぶことが一般的である.
* [デマルチプレクサの実装](https://masudahp.web.fc2.com/n88basic/kiso3/logic06.html)
	![デマルチプレクサ](../img/dmux.png)
<br />

<a id="multi-bit">

## 多ビットの基本ゲート
* コンピュータのハードウェアは、「バス」と呼ばれる複数のビットからなる配列を操作するように設計されている.
* 32ビットのコンピュータは、32ビットのバス2本に対してAND関数を行う必要がある. この操作を実装するためには、バイナリのANDゲートを32個用意し、各ゲートがペアとなるビットを個々に処理すればよい. この処理を行う回路は、一つのパッケージにまとめることができる. 32ビットの入力バスが2本、32ビットの出力バスが1本のインターフェースを持つ回路としてカプセル化することができる.
* 本書では16ビットのコンピュータを作ることを想定している.
* nビットの論理ゲートのアーキテクチャは、nの値に関係なく基本的に同じである.
* バスの各ビットについて言及する場合、配列のシンタックスを用いるのが一般的で、dataという名前の16ビットバスの各ビットにアクセスするには、data[0]、data[1]、...、data[15]という記述を用いる.
<br />

<a id="multi-input">

## 多入力の基本ゲート
* 2入力の論理ゲートを一般化すれば、多入力の論理ゲート、つまり任意の数の入力を受け入れることができる論理ゲートとなる.
<br />
