...
## 時間
* 順序回路は、1つ以上のDEF回路が直接または間接的に組み込まれている回路である. 機能面では、順序回路の機能（「状態を保つ（メモリなど）」「状態を操作する（カウンタなど）」）は、DEF回路によってもたらされる. 技術面では、順序回路内のフィードバックループによって先の機能は実現される.
* 組み合わせ回路においては、時間という概念はモデル化されていないため、フィードバックループを導入するには問題がある. なぜなら、出力は入力だけに依存するが、もしフィードバックループにより入力がその回路自身の出力に依存するとしたら、出力も出力自身に依存することになり、矛盾が生じるからである.
* 順序回路においては、DEFの「時間遅延」という性質（時刻tの出力ではなく、時刻t-1の出力がその回路自身に影響を与える）により、出力を同じ回路の入力に送信することは問題にならない. DEFのこの性質のおかげで、制御不能の「データレース」と呼ばれる現象（組み合わせ回路でフィールドバックループに伴う現象）を防止できる.
* この順序回路の出力が離散化される性質は、コンピュータアーキテクチャ全体を同期させることを実現する.
* コンピュータのクロックを作成する時、そのクロック周期の間隔を決めるだけに集中すればよい. 具体的には、アーキテクチャ内で最も距離が長い回路間を移動するのに必要な時間を調べ、それより少し長い時間をクロック周期の間隔とすればよい.
<br />
...
* Bit
	* 1ビットレジスタ（p43）.
	* $t-1$での出力$w0$と$in$を選択ビット$load$でMux回路に入れて、その出力をDFFに入れる.
* Register
	* 16ビットレジスタ（p44）.
	* Bitを16個並べる.
* RAM8
	* 8レジスタメモリ（p54、p55）.
	* Registerを8個並べる.
	* どのレジスタに$in$を読み書きするか決定するために、DMux8Way回路で$address$ごとに$load$を分割する（$load0-load7$）.
	* 各Registerに$in$と分割した$load$を入れて、各出力（$w0-w7$）を得る.
	* $address$の出力をMux8Way16回路に入れて求める.
* RAM64
	* 64レジスタメモリ（p55）.
	* RAM8を8個並べる.
	* RAM8の場合と同様の実装でよい. どのRAM8を選択しそのRAM8の中のどのレジスタを選択するかを決定するために、RAM8の選択に$address$の上位ビット、レジスタの選択に$address$の下位ビットを使用する.
* PC
	* カウンタ（p52、p53、p56）.
	* Inc16回路で$t-1$での出力$w0$にインクリメント処理した結果を出力する（$w1$）. $w0$と$w1$を選択ビット$incでMux16回路に入れて出力$w2$を得る. $w2$と$in$を選択ビット$load$でMux16回路に入れて出力$w3$を得る. $w3と0（false）を選択ビット$resetでMux16回路に入れて出力$w4$を得る. この出力結果$w4$に時間依存性を持たせて出力させるためにRegisterにload1（true）で入れる.
* RAM512
	* 512レジスタメモリ.
	* RAM64を8個並べる.
* RAM4K
